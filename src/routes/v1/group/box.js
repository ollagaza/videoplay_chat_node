...
routes.get('/', Auth.isAuthenticated(Role.LOGIN_USER), Wrap(async (req, res) => {
  const group_auth = await checkGroupAuth()
  const box_info_list = GroupSurgboxService.getGroupSurgboxInfoList(group_auth.group_seq)
  const output = new StdObject()
  output.add('box_info_list', box_info_list)
  res.json(output)
}))

routes.get('/check/:machine_id', Auth.isAuthenticated(Role.LOGIN_USER), Wrap(async (req, res) => {
  const group_auth = await checkGroupAuth()
  const is_duplicate = GroupSurgboxService.checkDuplicate(group_auth.group_seq, group_auth.machine_id)
  const output = new StdObject()
