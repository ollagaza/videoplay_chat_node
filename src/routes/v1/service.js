...
const on_error = Wrap(async(req, res) => {
  const query_str = querystring.stringify(req.query);
  log.d(req, '서비스 비디오 에러', query_str);

  const content_id = req.query.content_id;
  const sub_content_id = req.query.sub_content_id;
  const message = req.query.message;

  if (Util.isEmpty(content_id) || Util.isEmpty(sub_content_id)) {
    throw new StdObject(1, '잘못된 파라미터', 400);
  }
  ...
  if (operation_info.isEmpty()) {
    error_seq = await service_error_model.createServiceError('service_video', null, content_id, message);
  } else {
    await operation_model.updateRequestStatus(operation_info.seq, 'E');
    error_seq = await service_error_model.createServiceError('service_video', operation_info.seq, content_id, message);
  }
