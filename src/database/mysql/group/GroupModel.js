...
export default class GroupModel extends MySQLModel {
  ...
  getGroupInfoList = async (paging, search_option = null, search_keyword = null, order_field = null, order_type = null) => {
    ...
    const group_by_fields = ['group_info.seq', 'group_info.group_name', 'mem.user_name', 'group_info.reg_date', 'group_info.member_count', 'group_info.status']
    const query = this.database.select(select_fields)
    query.from(this.table_name)
    query.innerJoin('group_member as g_mem', (builder) => {
      builder.andOn(`${this.table_name}.seq`, 'g_mem.group_seq')
    })
    if (search_keyword) {
      switch (search_option) {
        case 'group_name':
          query.innerJoin('member as mem', (builder) => {
            builder.on(`${this.table_name}.member_seq`, 'mem.seq')
          })
