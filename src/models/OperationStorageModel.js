...
export default class OperationStorageModel extends ModelObject {
  ...
  getOperationStorageInfo = async (operation_seq) => {
    return new OperationStorageInfo(await this.findOne({operation_seq: operation_seq}));
  }
  ...
  updateUploadFileSize = async (storage_seq, file_type) => {
    ...
    if (file_type !== 'refer') {
      ...
      file_size_info = await new ReferFileModel({ database: this.database }).videoFileSummary(storage_seq);
      let total_size = file_size_info.total_size ? parseInt(file_size_info.total_size) : 0;
      total_size = Math.ceil(total_size / 1024 / 1024);
    ...
    await this.update({storage_seq: storage_seq}, update_params);
  ...
  updateStorageSummary = async (storage_seq) => {
    const update_params = {
      ...
      "total_file_count": this.database.raw('index1_file_count + index2_file_count + origin_video_count + trans_video_count + refer_file_count + service_video_count'),
