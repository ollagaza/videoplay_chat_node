...
const MemberLogServiceClass = class {
  ...
  createMemberLog = async (database,
       group_seq = null, member_seq = null, code, text = '', ip = null,
    ...
    return member_log_model.createMemberLog(group_seq, member_seq, code, text, ip, notice_page, notice_list, is_view)
  }

  memberJoinLog = async (database, member_seq) => {
    try {
      await this.createMemberLog(database, null, member_seq, "1000", '', null, 1)
      await this.createMemberLog(database, null, member_seq, "1001", 300, null, 1)
      await this.createMemberLog(database, null, member_seq, "8000", '', null, 1)
    } catch (error) {
  ...
  memberModifyLog = async (database, member_seq) => {
    try {
      await this.createMemberLog(database, null, member_seq, "1002", '', null, 1)
    } catch (error) {
  ...
  memberLeaveLog = async (database, member_seq, leave_text) => {
    try {
      await this.createMemberLog(database, null, member_seq, "9999", leave_text, null, 1)
    } catch (error) {
